...
<div class="max-w-2xl">
    <div class="bg-zinc-950 border border-white/5 rounded-2xl p-8">
        <form action="{{ role_route('classes.update', $class) }}" method="POST" class="space-y-5">
            @csrf @method('PUT')
            ...
            <div class="flex gap-3">
                ...
                <a href="{{ role_route('classes.index') }}" class="border border-white/10 text-gray-400 px-6 py-3 rounded-xl text-sm font-medium hover:border-white/30 transition-all">Batal</a>
            </div>
        </form>
    </div>
</div>
